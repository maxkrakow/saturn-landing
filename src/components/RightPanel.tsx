...
function RenewalScene() {
  ...
  React.useEffect(() => {
    ...
    const messageSequence = [
      { id: 'msg-1', type: 'system' as const, text: "Policy #GL-2024-001 for Tech Innovations LLC is set to expire in 10 days (01/15/2025).", timestamp: '2h ago', label: 'Policy set to expire', badges: [
        { text: 'Policy #GL-2024-001', variant: 'blue', icon: XIcon },
        { text: '10 days to expiry', variant: 'orange', icon: XIcon }
      ]},
      { id: 'msg-2', type: 'system' as const, text: "Called John Doe about the policy expiry. He asked for the portal link to upload the updated certificate.", timestamp: '2h ago', label: 'Called John Doe', badges: [
        { text: 'Phone Call', variant: 'gray', icon: XIcon },
        { text: 'Notes', variant: 'blue', icon: XIcon }
      ]},
      { id: 'msg-3', type: 'ai' as const, text: "Sent agent a link to the portal to upload the updated certificate", timestamp: '1h ago', name: 'AI Assistant', label: 'Emailed John Doe', badges: [
      ...
      { id: 'msg-4', type: 'system' as const, text: "John Doe uploaded a new file for policy #4782", timestamp: '1h ago', label: 'Document uploaded from John Doe', badges: [
        { text: 'PDF Policy_4782_Certificate', variant: 'red', icon: XIcon },
        { text: '3/5 Checks', variant: 'red', icon: XIcon }
      ]},
      { id: 'msg-5', type: 'system' as const, text: "Emailed agent with details on what the issues were, asking him to upload an updated certificate", timestamp: '1h ago', label: 'Emailed John Doe', badges: [
        { text: '@ Email', variant: 'blue', icon: XIcon },
      ...
      { id: 'msg-6', type: 'system' as const, text: "John Doe uploaded a new file for policy #4782", timestamp: '30m ago', label: 'Document uploaded from John Doe', badges: [
        { text: 'PDF Policy_4782_Certificate_v2', variant: 'green', icon: XIcon },
        { text: '5/5 Checks', variant: 'green', icon: XIcon }
      ]},
      { id: 'msg-7', type: 'ai' as const, text: "Policy is compliant. Emailing agent to let him know.", timestamp: '15m ago', name: 'AI Assistant', label: 'AI Assistant', badges: [
        { text: '@ Email', variant: 'blue', icon: XIcon }
      ], emailContent: "Hi John,\n\nPerfect! Everything looks good now. Thanks for the quick turnaround!\n\nYour policy is now fully compliant and ready to go.\n\nBest regards,\nBankwell Insurance Automation" },
      { id: 'msg-8', type: 'ai' as const, text: "Policy marked as compliant in system. Renewal process complete.", timestamp: '10m ago', name: 'AI Assistant', label: 'AI Assistant', badges: [
        { text: 'Compliant', variant: 'green', icon: BadgeCheckIcon }
      ]},
    ];
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 p-4 h-full"
    >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-sm font-medium text-gray-900">{(message as any).label || message.name}</span>
                        ...
                        {(message as any).emailContent && (
                          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-3 max-w-[400px]">
                    ...
                    <>
                      <div className="flex size-8 items-center justify-center rounded-full bg-gray-400">
                        <span className="text-white text-xs font-bold">{message.avatar || 'U'}</span>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                    ...
                    <>
                      <div className={`flex size-8 items-center justify-center rounded-full ${
                        (message as any).label === 'Policy set to expire' ? 'bg-orange-500' :
                        (message as any).label === 'Called John Doe' ? 'bg-green-500' :
                        (message as any).label === 'Document uploaded from John Doe' ? 'bg-blue-500' :
                        'bg-purple-500'
                      }`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-sm font-medium text-gray-900">{(message as any).label || 'System'}</span>
                          <span className="text-xs text-gray-500">{message.timestamp}</span>
                        </div>
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col">
        ...
        {/* ACORD Form */}
        <div ref={formRef} className="bg-white rounded-lg border border-gray-200 p-4 relative overflow-y-auto mb-4" style={{ height: '380px' }}>
          {/* Header */}
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="border border-gray-300 rounded p-3">
              <div className="text-gray-600 font-medium mb-2">Producer:</div>
              <div className={`h-5 rounded ${scannedFields.has('producer') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
            ...
            {/* Insured Section */}
            <div className="border border-gray-300 rounded p-3">
              <div className="text-gray-600 font-medium mb-2">Insured:</div>
              <div className={`h-5 rounded ${scannedFields.has('insured') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="border border-gray-300 rounded p-3">
                <div className="text-gray-600 font-medium mb-2">Policy #:</div>
                <div className={`h-5 rounded ${scannedFields.has('policy') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
                  {scannedFields.has('policy') && 'GL-2024-001'}
                </div>
              </div>
              <div className="border border-gray-300 rounded p-3">
                <div className="text-gray-600 font-medium mb-2">Effective Date:</div>
                <div className={`h-5 rounded ${scannedFields.has('dates') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
                  {scannedFields.has('dates') && '01/15/2024'}
                </div>
              </div>
              <div className="border border-gray-300 rounded p-3">
                <div className="text-gray-600 font-medium mb-2">Expiration Date:</div>
                <div className={`h-5 rounded ${scannedFields.has('dates') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
                  {scannedFields.has('dates') && '01/15/2025'}
                </div>
              </div>
              <div className="border border-gray-300 rounded p-3">
                <div className="text-gray-600 font-medium mb-2">Limits:</div>
                <div className={`h-5 rounded ${scannedFields.has('limits') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
            ...
            {/* Coverage Section */}
            <div className="border border-gray-300 rounded p-3">
              <div className="text-gray-600 font-medium mb-2">Coverage Type:</div>
              <div className={`h-5 rounded ${scannedFields.has('coverage') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
            ...
            {/* Additional Insured */}
            <div className="border border-gray-300 rounded p-3">
              <div className="text-gray-600 font-medium mb-2">Additional Insured:</div>
              <div className={`h-5 rounded ${scannedFields.has('additional') ? 'bg-red-50 text-red-900 px-2 py-1' : 'bg-gray-200'}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="border border-gray-300 rounded p-3">
                <div className="text-gray-600 font-medium mb-2">Waiver of Subrogation:</div>
                <div className={`h-5 rounded ${scannedFields.has('waiver') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
                  {scannedFields.has('waiver') && 'Yes'}
                </div>
              </div>
              <div className="border border-gray-300 rounded p-3">
                <div className="text-gray-600 font-medium mb-2">Certificate Holder:</div>
                <div className={`h-5 rounded ${scannedFields.has('holder') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
            ...
            <div className="border border-gray-300 rounded p-3">
              <div className="text-gray-600 font-medium mb-2">Endorsements:</div>
              <div className="space-y-2">
                <div className={`h-4 rounded ${scannedFields.has('endorsements') ? 'bg-blue-50 text-blue-900 px-2 py-1' : 'bg-gray-200'}`}>
          ...
          {!scanComplete && (
            <div
              className="absolute left-0 h-1 w-full bg-blue-500 shadow-lg z-20 opacity-80"
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col">
        ...
        {/* Chart Area */}
        <div className="flex-1 bg-gray-50 rounded-lg p-4 mb-4 min-h-0">
          <div className="text-sm text-gray-700 mb-3 font-medium">Compliance Trends (30 days)</div>
          <div className="h-20 flex items-end gap-1">
            {[65, 72, 68, 75, 78, 82, 85, 88, 91, 89, 93, 95].map((height, i) => (
              <motion.div
                key={i}
                className="flex-1 bg-blue-500 rounded-t shadow-sm"
                initial={{ height: 0 }}
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="text-sm text-gray-700 mb-3 font-medium">Recent Activity</div>
          <div className="space-y-2">
            <ActivityItem 
              text="Policy GL-2024-001 renewed" 
              time="2m ago" 
              status="success"
              delay={0.8}
            />
            <ActivityItem 
              text="Missing endorsement flagged" 
              time="5m ago" 
              status="warning"
              delay={0.9}
            />
            <ActivityItem 
              text="Broker contacted for ABC Corp" 
              time="12m ago" 
              status="info"
              delay={1.0}
            />
            <ActivityItem 
              text="SLA breach warning for Tech Corp" 
              time="15m ago" 
              status="warning"
              delay={1.1}
            />
...
/* ---------- Building blocks ---------- */
function Notification({ icon, title, desc, status, delay }: { 
  icon: string; 
  title: string; 
  desc: string; 
  ...
  return (
    ...
    >
      ...
      <div className={`w-2 h-2 rounded-full ${status === "success" ? "bg-green-400" : "bg-yellow-400"}`} />
    </motion.div>
  );
}

function FormField({ label, value, highlight, missing }: { 
  label: string; 
  value: string; 
  highlight?: boolean; 
  missing?: boolean;
...
function AIMessage({ text, delay }: { text: string; delay: number }) {
  return (
    ...
    >
      ...
      <div className="bg-blue-50 rounded-lg px-2 py-1 text-xs text-gray-800 max-w-[200px] leading-tight">
        {text}
      </div>
    </motion.div>
  );
...
function UserMessage({ text, delay }: { text: string; delay: number }) {
  return (
    ...
    >
      <div className="bg-blue-500 rounded-lg px-2 py-1 text-xs text-white max-w-[200px] leading-tight">
        {text}
      </div>
      <div className="w-5 h-5 bg-gray-500 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
        U
      </div>
    </motion.div>
  );
...
function AutomationStep({ icon, title, desc, status, delay }: { 
  icon: string; 
  title: string; 
  desc: string; 
  ...
  return (
    ...
    >
      ...
      <Badge variant={statusVariants[status]} icon={status === "completed" ? BadgeCheckIcon : status === "scheduled" ? AlertTriangleIcon : XIcon}>
        {status}
      </Badge>
    </motion.div>
  );
...
function ActivityItem({ text, time, status, delay }: { 
  text: string; 
  time: string; 
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <Badge variant={statusVariants[status]} icon={status === "success" ? BadgeCheckIcon : status === "warning" ? AlertTriangleIcon : XIcon}>
          {text}
        </Badge>
      </div>
